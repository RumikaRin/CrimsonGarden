...
async function main() {
  ...
  // 1. Delete all existing records (Cascade delete handles child relations)
  ...
  const studentPasswordHash = hashSync('student123', 10);
  const student = await prisma.user.create({
    data: {
      email: 'student@crimson.edu.vn',
      name: 'Crimson Student',
