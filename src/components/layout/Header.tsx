        ...
        <div style={{ borderWidth: 3 }} className={`${containerBorderClass} rounded-2xl sm:rounded-3xl px-3 sm:px-6 lg:px-10 h-20 flex items-center justify-between transition-all duration-300 max-w-[100rem] mx-auto`}>
          ...
          <div className="flex items-center gap-3">
            <span className="font-serif text-2xl tracking-tight font-bold">
              ...
              <span className="italic font-normal font-serif text-[var(--accent)]">Garden</span>
            </span>
          ...
          <nav className="hidden md:flex items-center gap-1 bg-[#1A1814]/5 p-1 rounded-xl border border-[#1A1814]/5">
            <motion.button onClick={() => router.push('/')} className={navTabClass('/')} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }}>Trang Chủ</motion.button>
            <motion.button onClick={() => router.push('/quiz')} className={navTabClass('/quiz')} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }}>Ôn Thi Trắc Nghiệm</motion.button>
            <motion.button onClick={() => router.push('/snake')} className={navTabClass('/snake')} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }}>Săn Từ Vựng</motion.button>
            <motion.button onClick={() => router.push('/leaderboard')} className={navTabClass('/leaderboard')} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }}>Bảng Xếp Hạng</motion.button>
            <motion.button onClick={() => router.push('/generate')} className={navTabClass('/generate')} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }}>Bóc Tách Đề</motion.button>
          </nav>

          {/* Right: Theme + Login button */}
          <div className="flex items-center gap-3">
            <div className="flex items-center bg-[#1A1814]/5 p-0.5 rounded-lg border border-[#1A1814]/5">
              <button onClick={() => setTheme('cozy')} className={cn('px-2 py-1.5 rounded-md text-[9px] font-sans font-bold uppercase tracking-wider transition-all', !isGreenTheme ? 'bg-[var(--accent)] text-white shadow-sm' : 'text-[var(--text-secondary)] opacity-60 hover:opacity-100')}>Crimson</button>
              <button onClick={() => setTheme('neon')} className={cn('px-2 py-1.5 rounded-md text-[9px] font-sans font-bold uppercase tracking-wider transition-all', isGreenTheme ? 'bg-[var(--accent)] text-white shadow-sm' : 'text-[var(--text-secondary)] opacity-60 hover:opacity-100')}>Garden</button>
            </div>
            <button
              onClick={() => router.push('/login')}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-sans font-bold uppercase tracking-widest bg-[var(--accent)] text-white shadow-sm hover:shadow-md transition-all cursor-pointer active:scale-[0.97]"
            >
              Đăng nhập
            </button>
          </div>
