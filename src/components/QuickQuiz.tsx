    ...
    return (
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <section className="flex flex-col gap-4 rounded-2xl border border-[var(--border-default)] bg-[var(--card-bg)] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7">
          ...
          <button type="button" disabled={questionPool.length === 0} onClick={startQuiz} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--accent)] px-6 text-xs font-bold uppercase tracking-wider text-white disabled:opacity-40">
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <section className="rounded-2xl border border-[var(--border-default)] bg-[var(--card-bg)] p-5 sm:p-8">
        ...
        {selectedId && <div className="mt-6 border-t border-[var(--border-default)] pt-5"><p className="text-xs leading-5 text-[var(--text-secondary)]">{question.explanation || 'Ghi nhớ đáp án đúng và tiếp tục câu kế tiếp.'}</p><button type="button" onClick={nextQuestion} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--accent)] px-5 text-xs font-bold uppercase tracking-wider text-white">{index === questions.length - 1 ? 'Xem kết quả' : 'Câu tiếp theo'} <ArrowRight className="h-4 w-4" /></button></div>}
      </section>
    </div>
  );
}
